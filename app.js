...
const MASTER_VIBE_TAGS = ["Bar","Party","Cinema","Sauna","Shop","Cruising","Darkroom","Men Only","Dresscode","Naked","Underwear","Dancefloor","Smoking Area","Cocktails","Fetish/Gear","Bear","Mature","Young Crowd","Queer","Pride","Drag","Karaoke","Pop/Dance","Techno"];
...
function getKnownCityName(value, source = venues) {
    const target = normalizeCityName(value);
    if (!target) return '';
    for (const venue of (source || [])) {
        const match = getCityTokens(venue).find(city => normalizeCityName(city) === target);
...
function parseLocationQuery(value) {
    ...
    if (!raw) return { scope: 'all', city: '', country: '', raw: '' };

    const parts = raw.split(',').map(part => part.trim()).filter(Boolean);
    if (parts.length >= 2) {
        const city = getKnownCityName(parts[0]) || parts[0];
        const countryRaw = parts.slice(1).join(', ');
        const country = getKnownCountryName(countryRaw) || countryRaw;
        return { scope: 'city', city, country, raw };
    }

    const single = parts[0] || raw;
    const knownCity = getKnownCityName(single);
    const knownCountry = getKnownCountryName(single);

    // City takes precedence where the same word could be both a city and a country.
    if (knownCity) return { scope: 'city', city: knownCity, country: '', raw };
    if (knownCountry) return { scope: 'country', city: '', country: knownCountry, raw };
    return { scope: 'city', city: single, country: '', raw };
}
...
function getSavedLocationScope(location = getSavedLocation()) {
    if (!location || typeof location !== 'object') return { scope: 'all', city: '', country: '', postcode: '', label: '' };

    const city = String(location.city || '').trim();
    const country = String(location.country || '').trim();
    const postcode = String(location.postcode || '').trim();
    const requestedScope = String(location.scope || '').trim().toLowerCase();

    if (requestedScope === 'all' || (!city && !country)) return { scope: 'all', city: '', country: '', postcode, label: '' };
    if (city && city.toLowerCase() !== 'my location') return { scope: 'city', city, country, postcode, label: city };
    if (country) return { scope: 'country', city: '', country, postcode, label: country };
    return { scope: 'all', city: '', country: '', postcode, label: '' };
}

function venueMatchesSavedLocation(venue, location = getSavedLocationScope()) {
    if (location.scope === 'country') return venueMatchesCountry(venue, location.country);
    if (location.scope === 'city') {
        return venueMatchesCity(venue, location.city)
            && (!location.country || venueMatchesCountry(venue, location.country));
    }
    return true;
}

function formatLocationQuery(location) {
    if (!location || location.scope === 'all') return '';
    if (location.scope === 'country') return location.country || '';
    return [location.city, location.country].filter(Boolean).join(', ');
}

function updateLocationActionLabel(value) {
    const button = document.getElementById('btn-save-location');
    if (!button) return;
    const location = parseLocationQuery(value ?? document.getElementById('loc-query')?.value ?? '');
    const label = location.scope === 'all' ? 'All Cities' : (location.label || location.city || location.country);
    button.textContent = `Show Results in ${label}`;
}
...
window.setBackroomSharedLocation = function(location) {
    const next = getSavedLocationScope(location);
    const stored = {
        scope: next.scope,
        city: next.city,
    ...
    localStorage.setItem('br_location', JSON.stringify(stored));
    updateLocationDisplay(stored);
    announceLocationChange(stored);
};
...
function setupEventListeners() {
    ...
    if (locationQueryInput && !locationQueryInput.dataset.locationBound) {
        locationQueryInput.dataset.locationBound = 'true';
        locationQueryInput.addEventListener('input', () => updateLocationActionLabel(locationQueryInput.value));
        locationQueryInput.addEventListener('change', () => updateLocationActionLabel(locationQueryInput.value));
    }
    updateLocationActionLabel();
    ...
    addEvt('btn-search-map', 'click', async () => {
        const location = document.getElementById('loc-query')?.value.trim() || '';
        const pc = document.getElementById('loc-postcode')?.value.trim() || '';
        const query = `${pc} ${location}`.trim();
    ...
    addEvt('btn-save-travel', 'click', () => {
        const locationInp = document.getElementById('loc-query');
        if(!locationInp) return;
        const city = parseLocationQuery(locationInp.value).city;
...
function applyFilters() {
    ...
    const locationScope = getSavedLocationScope();
    let filteredVenues = getPublicVenues().filter(venue => venueMatchesSavedLocation(venue, locationScope));
    selectedCardId = null;
    ...
    if(contextHeader) {
        ...
        const locationHeading = locationScope.scope === 'all' ? 'RESULTS' : `RESULTS IN ${String(locationScope.label || locationScope.city || locationScope.country).toUpperCase()}`;
        if(title) title.innerText = parts.length ? `${locationHeading} — ${parts.join(' + ')}` : locationHeading;
...
function saveLocation() {
    ...
    const parsed = parseLocationQuery(queryInput?.value || '');
    const loc = {
        scope: parsed.scope,
...
function clearLocation() {
    ...
    if(queryInput) {
        queryInput.value = '';
        queryInput.dataset.lat = '';
        queryInput.dataset.lon = '';
    }
    if(postcodeInput) postcodeInput.value = '';
    if(map) map.style.display = 'none';
    if(placeholder) placeholder.classList.remove('hidden');

    const all = { scope: 'all', country: '', city: '', postcode: '' };
    updateLocationDisplay(all);
    updateLocationActionLabel('');
    announceLocationChange(all);
    if (window.location.hash === '#results') applyFilters();
}
...
function loadSavedLocation() {
    ...
    if(queryInput) queryInput.value = formatLocationQuery(scope);
    if(postcodeInput) postcodeInput.value = scope.postcode || '';
    updateLocationDisplay(scope);
    updateLocationActionLabel(queryInput?.value || '');
}
...
function updateLocationDisplay(location) {
    ...
    if(!display) return;
    const scope = getSavedLocationScope(location);
    if (scope.scope === 'country') display.innerText = `Current country: ${scope.country}`;
    else if (scope.scope === 'city') display.innerText = `Current location: ${formatLocationQuery(scope)}`;
    else display.innerText = 'No location set. Showing all cities and countries.';
}
